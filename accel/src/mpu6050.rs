...
impl Mpu6050 {
    pub fn new(mut i2c: I2c, address: u8) -> Result<Mpu6050, Error> {
        let whoami = read_byte(&mut i2c, address, WHOAMI_REG)?;
        if whoami != WHOAMI {
            return Err(Error::Identification)
        }

        // reset device
        write_byte(&mut i2c, address, PWR_MGMT_1, 0x80)?;
        delay_ms(100);
        // wake up device
        write_byte(&mut i2c, address, PWR_MGMT_1, 0x00)?;
        delay_ms(100);

        // get stable time source
        write_byte(&mut i2c, address, PWR_MGMT_1, 0x03)?;
        delay_ms(200);

        // configure gyro and thermometer
        write_byte(&mut i2c, address, MPU_CONFIG, 0x03)?;
        // set sample rate
        write_byte(&mut i2c, address, SMPLRT_DIV, 0x03)?;

        let mut c= read_byte(&mut i2c, address,  GYRO_CONFIG)?;
        c = c & !0x03; // Clear Fchoice bits [1:0]
        c = c & !0x18; // Clear GFS bits [4:3]
        c = c | 3 << 3; // Set 2000dps full scale range for the gyro (11 on 4:3)
        write_byte(&mut i2c, address,  GYRO_CONFIG,c)?;

        write_byte(&mut i2c, address, MPU_CONFIG,  0x03)?;

        write_byte(&mut i2c, address,  INT_PIN_CFG,0x22)?;
        write_byte(&mut i2c, address, INT_ENABLE,0x01)?;

        delay_ms(100);
        return Ok(Mpu6050 { i2c, address, calibration: CalData::default() });
    }
    ...
    pub fn update(&mut self) -> Result<MpuData, Error> {
        ...
        let gx = (imu_count[4] as f32) * A_RES - self.calibration.gyro_bias[0];
        let gy = (imu_count[5] as f32) * A_RES - self.calibration.gyro_bias[1];
        let gz = (imu_count[6] as f32) * A_RES - self.calibration.gyro_bias[2];
    ...
    pub fn calibrate(&mut self) -> Result<(), Error> {
        ...
        let mut gyro_bias: [f32; 3] = [0.; 3];
        let mut accel_bias: [f32; 3] = [0.; 3];
